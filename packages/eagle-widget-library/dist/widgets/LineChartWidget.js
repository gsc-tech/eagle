...
export const LineChartWidget = ({ apiUrl = "", title, parameters, dateField = "date", xAxisType = "date", seriesConfig = DEFAULT_SERIES_CONFIG, groupedParametersValues, onGroupedParametersChange, darkMode = false, sheetDependency, }) => {
    const chartId = useId();
    ...
    let routeData = null;
    if (apiUrl != "") {
        routeData = useWidgetData(apiUrl, {
            ...
            try {
                ...
                if (seriesRefs.current.length > 0 && rawData && Array.isArray(rawData)) {
                    ...
                    if (xAxisType === "date") {
                        ...
                        processedData.sort((a, b) => new Date(a[dateAxis]).getTime() - new Date(b[dateAxis]).getTime());
    ...
    useEffect(() => {
        if (seriesRefs.current.length > 0 && rawData && Array.isArray(rawData)) {
            ...
            if (xAxisType === "date") {
                // Sort data by date
                processedData.sort((a, b) => new Date(a[dateField]).getTime() - new Date(b[dateField]).getTime());
                // Convert date strings to timestamps
                processedData = processedData.map((item) => {
                    const mappedItem = { ...item };
    ...
    return (_jsx(WidgetContainer, { title: title, parameters: parameters, onParametersChange: handleParametersChange, darkMode: darkMode, onGroupedParametersChange: onGroupedParametersChange, groupedParametersValues: groupedParametersValues, children: _jsx("div", { id: chartId, ref: containerRef, className: "w-full h-full" }) }));
};
