...
export const ParameterForm = ({ parameters, onParametersChange, darkMode = false, groupedParametersValues, onGroupedParametersChange, }) => {
    ...
    const renderInput = (param) => {
        ...
        switch (param.type) {
            case 'text':
                return (_jsx("input", { type: "text", id: param.name, value: currentValue, onChange: (e) => handleChange(param.name, e.target.value, param.groupId), placeholder: param.placeholder || param.label, required: param.required, className: baseClasses }));
            case 'number':
                return (_jsx("input", { type: "number", id: param.name, value: currentValue, onChange: (e) => handleChange(param.name, e.target.value ? Number(e.target.value) : '', param.groupId), placeholder: param.placeholder || param.label, required: param.required, className: baseClasses }));
            case 'date':
                return (_jsx("input", { type: "date", id: param.name, value: currentValue, onChange: (e) => handleChange(param.name, e.target.value, param.groupId), required: param.required, className: baseClasses }));
            case 'select':
                return (_jsxs("select", { id: param.name, value: currentValue, onChange: (e) => handleChange(param.name, e.target.value, param.groupId), required: param.required, className: `${baseClasses} pr-6`, children: [_jsx("option", { value: "", children: param.label }), param.options?.map(option => (_jsx("option", { value: option.value, children: option.label }, option.value)))] }));
            case 'checkbox':
                return (_jsx("input", { type: "checkbox", id: param.name, checked: currentValue || false, onChange: (e) => handleChange(param.name, e.target.checked, param.groupId), className: "w-3.5 h-3.5 text-chart-primary border-border-light rounded focus:ring-chart-primary cursor-pointer" }));
            default:
                return null;
        }
    };
    return (_jsx("form", { className: `flex flex-wrap items-center gap-2 relative z-30 ${darkMode
            ? 'bg-transparent'
            : 'bg-transparent'}`, children: parameters.map(param => (_jsxs("div", { className: "flex items-center gap-1.5 group relative", children: [param.type !== 'checkbox' && (_jsx("label", { htmlFor: param.name, className: `text-[10px] font-medium whitespace-nowrap transition-colors ${darkMode
                        ? 'text-gray-400 group-hover:text-gray-200'
                        : 'text-text-secondary group-hover:text-chart-primary'}`, children: param.label })), renderInput(param), param.type === 'checkbox' && (_jsx("label", { htmlFor: param.name, className: `text-xs cursor-pointer select-none ${darkMode ? 'text-gray-400' : 'text-text-secondary'}`, children: param.label })), groupedParametersValues !== undefined && (_jsxs("div", { className: "flex items-center", children: [_jsx("button", { type: "button", onClick: () => setActiveGroupParam(activeGroupParam === param.name ? null : param.name), title: param.groupId ? `Group: ${param.groupId}` : 'Attach to Group', className: `p-1 rounded flex items-center justify-center transition-all duration-200 ${param.groupId
                                ? 'text-chart-primary bg-chart-primary/10'
                                : 'text-gray-400 opacity-0 group-hover:opacity-100 hover:bg-gray-100 dark:hover:bg-gray-800'}`, children: _jsxs("svg", { width: "10", height: "10", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "3", "stroke-linecap": "round", "stroke-linejoin": "round", children: [_jsx("path", { d: "M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" }), _jsx("path", { d: "M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" })] }) }), activeGroupParam === param.name && (_jsx("div", { className: `absolute z-50 top-full mt-1 p-2 rounded-md shadow-xl border min-w-[140px] animate-in fade-in slide-in-from-top-1 duration-200 ${darkMode
                                ? 'bg-gray-800 border-gray-700'
                                : 'bg-white border-gray-200'}`, children: _jsxs("div", { className: "flex flex-col gap-2", children: [_jsxs("div", { children: [_jsx("div", { className: "text-[9px] font-semibold mb-1 uppercase tracking-wider opacity-50", children: "Create New" }), _jsxs("div", { className: "flex gap-1", children: [_jsx("input", { type: "text", value: newGroupId, onChange: (e) => setNewGroupId(e.target.value), placeholder: "Group ID", className: `text-[10px] px-1.5 py-0.5 border rounded w-full focus:outline-none ${darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`, onKeyDown: (e) => {
                                                        ...
                                                        }, className: "bg-chart-primary text-white text-[10px] px-1.5 rounded", children: "+" })] })] }), groupedParametersValues && Object.keys(groupedParametersValues).length > 0 && (_jsxs("div", { children: [_jsx("div", { className: "text-[9px] font-semibold mb-1 uppercase tracking-wider opacity-50", children: "Existing Groups" }), _jsx("div", { className: "flex flex-col gap-0.5 max-h-[80px] overflow-y-auto", children: Object.keys(groupedParametersValues).map(gid => (_jsx("button", { type: "button", onClick: () => {
                                                        param.groupId = gid;
                                                        setActiveGroupParam(null);
                                                        ...
                                                        : darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`, children: gid }, gid))) })] })), param.groupId && (_jsx("button", { type: "button", onClick: () => {
                                            param.groupId = undefined;
                                            setActiveGroupParam(null);
                                        }, className: "text-[9px] text-red-500 hover:text-red-600 text-left mt-1 pt-1 border-t border-gray-100 dark:border-gray-700", children: "Detach from Group" }))] }) }))] }))] }, param.name))) }));
};
