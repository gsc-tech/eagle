...
export const MarketDepthWidget: React.FC<MarketDepthWidgetProps & { darkMode?: boolean }> = ({
    ...
    return (
        ...
        >
            <div className={`flex items-center h-full w-full px-2 py-1 ${darkMode ? 'bg-[#141414]' : 'bg-white'}`}>
                <div className="flex items-center w-full gap-2">
                    ...
                    <div className={`flex-1 flex items-center justify-between px-3 py-1.5 rounded-lg border relative overflow-hidden group ${darkMode
                        ...
                        <div className="flex flex-col">
                            <span className={`text-[10px] uppercase font-semibold tracking-wider ${darkMode ? 'text-[#909090]' : 'text-text-secondary'}`}>Bid</span>
                            <span className="font-mono font-bold text-bid text-base leading-none">
                                {formatNumber(l1Data.bestBid, 2)}
                            </span>
                        </div>
                        <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${darkMode ? 'text-[#e0e0e0] bg-[#1a1a1a]/50' : 'text-text-secondary bg-white/50'}`}>
                    ...
                    <div className="flex flex-col items-center justify-center px-2 min-w-[60px]">
                        <div className="flex items-center gap-1 mb-0.5">
                            ...
                            <span className={`text-[10px] font-bold ${darkMode ? 'text-[#606060]' : 'text-text-muted'}`}>SPREAD</span>
                            <ArrowRight size={10} className="text-ask" />
                        </div>
                        <span className={`font-mono text-xs font-bold ${darkMode ? 'text-[#f0f0f0]' : 'text-text-primary'}`}>
                            {formatNumber(spread, 2)}
                        </span>
                        {spreadPercentage !== null && (
                            <span className={`text-[9px] font-mono ${darkMode ? 'text-[#909090]' : 'text-text-secondary'}`}>
                    ...
                    <div className={`flex-1 flex items-center justify-between px-3 py-1.5 rounded-lg border relative overflow-hidden group ${darkMode
                        ...
                        <div className="flex flex-col">
                            <span className={`text-[10px] uppercase font-semibold tracking-wider ${darkMode ? 'text-[#909090]' : 'text-text-secondary'}`}>Ask</span>
