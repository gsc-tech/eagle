...
export const ScatterPlotWidget = ({ apiUrl = "http://localhost:8080/api/data", title, parameters, xField = "x", yField = "y", sizeField = "size", categoryField = "category", pointColor = "#6366f1", pointSize = 10, showTrendLine = false, enableZoom = true, darkMode = false, groupedParametersValues, onGroupedParametersChange, }) => {
            ...
            try {
                ...
                if (hasCategories) {
                    ...
                    if (showTrendLine && data && data.length > 1) {
                        const trendSeries = chart.series.push(am5xy.LineSeries.new(root, {
                            xAxis: xAxis,
                            yAxis: yAxis,
                            valueYField: "trendY",
                            valueXField: "trendX",
                        ...
                        const n = data.length;
                        const sumX = data.reduce((sum, item) => sum + item[xField], 0);
                        const sumY = data.reduce((sum, item) => sum + item[yField], 0);
                        const sumXY = data.reduce((sum, item) => sum + (item[xField] * item[yField]), 0);
                        const sumX2 = data.reduce((sum, item) => sum + (item[xField] * item[xField]), 0);
                        const slope = (n * sumXY - sumX * sumY) / (n * sumX2 - sumX * sumX);
                        const intercept = (sumY - slope * sumX) / n;
                        const minX = Math.min(...data.map((item) => item[xField]));
                        const maxX = Math.max(...data.map((item) => item[xField]));
                        const trendData = [
